...
ExcecaoCelulaInvalida::ExcecaoCelulaInvalida(int linha, int coluna) {
  _linha = linha;
  _coluna = coluna;
  mensagem = "Célula (" + std::to_string(linha) + "," + std::to_string(coluna) + ") não é válida. Deseja continuar e ignorá-lá? (s/n)?";
}

const char* ExcecaoCelulaInvalida::what() const noexcept{
  char* palavra = new char [mensagem.size() + 1];
  strcpy (palavra, mensagem.c_str());
  return palavra;
};
...
void JogoDaVida::Reviver(int i, int j) noexcept(false) {
